...
class Sha256:#creating a class fro sha256
    ks = [
    ...
    ]#storing the keys in a array for initializing buffers from k[0] to k[63]

    hs = [
        0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a,
        0x510e527f, 0x9b05688c, 0x1f83d9ab, 0x5be0cd19,
    ] #initializing 8 buffers

    M32 = 0xFFFFFFFF

    def __init__(self, m = None):#constructor class
        self.mlen = 0#length intilization
        self.buf = b''#buffer initialization
        self.k = self.ks[:]#key value intilization
        self.h = self.hs[:]#hash buffer intialization
        self.fin = False
        if m is not None:
            self.update(m)

    @staticmethod
    def pad(mlen):#padding the input string
        mdi = mlen & 0x3F
    ...
    def compress(self, c):#the entire meassage gets broken down into multiple blocksof 512 bits each.
        w = [0] * 64
        ...
        for i in range(64):#each block goes through 64 rounds, output of each block serving as the input for following bock.
        ...
        #the entire cycle repeats until you reach 512 bit block    
